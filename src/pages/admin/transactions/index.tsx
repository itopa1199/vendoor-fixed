...
export default function TransactionsPage() {
  ...
  const load = () => {
    ...
    adminDashboard.getStats()
      .then(r => {
        console.log('[Transactions] stats response:', r)
        const list = Array.isArray(r.data?.transactions) ? r.data.transactions :
  ...
  const norm = (t: any) => ({
    ref:    t.reference ?? t.payment_ref ?? t.ref ?? '—',
    vendor: t.vendor_name ?? t.store_name ?? t.vendor ?? '—',
    type:   t.type ?? t.transaction_type ?? 'product',
    amount: Number(t.amount ?? t.total ?? 0),
    cut:    Number(t.platform_cut ?? t.commission ?? t.cut ?? 0),
    date:   (t.created_at ?? t.date ?? '').slice(0, 10),
    status: t.status ?? t.payment_status ?? 'success',
    raw: t,
  })

  const transactions = all.map(norm)
  const products  = transactions.filter(t => t.type === 'product')
  const spotlight = transactions.filter(t => t.type === 'spotlight')
  const refunded  = transactions.filter(t => t.status === 'refunded')

  const visible = tab === 'product' ? products : tab === 'spotlight' ? spotlight : tab === 'refunded' ? refunded : transactions

  const totalVol  = transactions.reduce((s, t) => s + t.amount, 0)
  const totalCuts = transactions.reduce((s, t) => s + t.cut,    0)
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <StatCard icon={<BarChart2 size={18} />}   value={String(transactions.length)} label="Total Transactions"    accent="purple" />
      </div>
      ...
      <Card>
        ...
        <div className="overflow-x-auto">
          <table>
            <thead>
              <tr><th>Reference</th><th>Vendor</th><th>Type</th><th>Amount</th><th>Platform Cut</th><th>Date</th><th>Status</th></tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={7} className="py-10">
                  <Empty icon={<DollarSign size={18} />} title="No transactions" sub="Transactions will appear here once loaded from the API" />
                </td></tr>
              ) : visible.map((t, i) => (
                <tr key={i}>
                  <td className="font-mono text-[11px] font-semibold text-[#2563EB]">{t.ref}</td>
                  <td className="font-semibold">{t.vendor}</td>
                  <td><Badge v={t.type === 'spotlight' ? 'gold' : 'green'}>{t.type}</Badge></td>
